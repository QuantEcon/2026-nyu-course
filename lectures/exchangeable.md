...
An example  that we study in this lecture  is a key component of {doc}`this lecture <intermediate:odu>` that augments the
{doc}`classic <intermediate:mccall_model>`  job search model of McCall
...
We'll use such graphs to provide insights into  mechanics driving outcomes in {doc}`this lecture <intermediate:odu>` about learning in an augmented McCall job
search model.
