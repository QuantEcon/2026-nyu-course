...
As we learned in {doc}`intermediate:prob_matrix` and  {doc}`intermediate:prob_meaning`, a frequentist statistician views a probability distribution as measuring relative frequencies of a statistic that he anticipates constructing  from a very long sequence of i.i.d. draws from a known probability distribution.  
...
As in {doc}`intermediate:os_stochastic`, to approximate a continuous value function
